...
const EmailUpdate = () => {
  ...
  const submitHandler = async (event) => {
    ...
    if (enteredNewEmail.trim().length < 1) {
      setErrorRequest("Enter your new email");
      newEmailInputRef.current.value = "";
      setCorrectRequest(false);
      return;
    } else if (!enteredNewEmail.includes("@")) {
      setErrorRequest("Incorrect email format (requires @)");
      newEmailInputRef.current.value = "";
      setCorrectRequest(false);
      return;
    } else if (!enteredNewEmail.includes(".com")) {
      setErrorRequest("Incorrect email format (requires .com)");
      newEmailInputRef.current.value = "";
      setCorrectRequest(false);
      return;
    } else if (!emailFormat(enteredNewEmail)) {
      setErrorRequest("Incorrect email format (no spaces allowed)");
      newEmailInputRef.current.value = "";
      setCorrectRequest(false);
      return;
    } else if (enteredNewEmail === profileCtx.email) {
      setErrorRequest("You already have that email");
      newEmailInputRef.current.value = "";
      setCorrectRequest(false);
      return;
    ...
    //Si pasa las anteriores, mandamos request.
    setIsLoading(true);
        ...
        <div className="mx-auto my-0 flex  max-w-[600px] grow flex-col">
          ...
          <div className="flex flex-col rounded-[8px] border border-solid border-[#D5D9D9] px-[18px] py-[14px] font-sans">
            <div className="w-full text-[13px]">
              If you want to change the email associated with your Site customer
            ...
            <div className="mt-[22px] w-full">
              <form onSubmit={submitHandler}>
                ...
                <div className="mb-[22px]">
                  <input
                    className="m-[1px] w-[300px] rounded-[3px] border border-solid border-[#a6a6a6] px-[7px] py-[3px] ring-borderRingLogin  focus:border focus:border-borderLogin focus:outline-none focus:ring"
